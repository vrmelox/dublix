...
// Import the types - make sure this path is correct
...
export const authMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Vérifier et décoder le token avec proper error handling
...
export const rateLimit = (maxRequests: number = 100, windowMs: number = 15 * 60 * 1000) => {
  return (req: Request, res: Response, next: NextFunction) => {
    // Fix for the userId type error - provide fallback
...
// export const requireAdminOrTechnicien = requireRole(['ADMIN', 'TECHNICIEN']);